...
const { scrapeCoursePyqLinks } = require('./pyq-links');
const { scrapeEduRev, toSlug } = require('./scraper');
  ...
  for (const chapterEntry of pyqIndex.chapters) {
    ...
    for (const pyq of pyqLinks) {
      ...
      const scrapeResult = await scrapeEduRev(pyq.canonicalUrl || pyq.url, {
        ...
        classCode: chapter.classCode || chapter.class
      });
...
if (require.main === module) {
  scrapeAllChapters(source).catch(() => {
    process.exitCode = 1;
  });
}
