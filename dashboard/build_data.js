...
const CHAPTER_DIR = path.join(RAW_DIR, 'BIOLOGY', 'chapters');
...
function buildQuestionSummary(question, chapterSlug) {
  ...
  return {
    ...
    images: images.map((imagePath, index) => ({
      id: `${chapterSlug}-${question.id}-image-${index + 1}`,
      path: imagePath,
...
function buildChapterRecord(filePath, pyqEntry, auditEntry, residualEntry, alignmentEntry, schemaFailures) {
  ...
  const questionSummaries = questions.map((question) => buildQuestionSummary(question, slug));
...
function main() {
  ...
  const pyqBySlug = new Map(
    (pyqIndex.chapters || []).map((entry) => [toSlug(entry.chapter && entry.chapter.title), entry])
  );

  const chapterFiles = fs.existsSync(CHAPTER_DIR)
    ? fs
        .readdirSync(CHAPTER_DIR)
        .filter((fileName) => fileName.endsWith('.json'))
        .sort()
        .map((fileName) => path.join(CHAPTER_DIR, fileName))
    : [];
  ...
  const orphanPyqEntries = (pyqIndex.chapters || [])
    .filter((entry) => !chapters.some((chapter) => chapter.slug === toSlug(entry.chapter && entry.chapter.title)))
    .map((entry) => ({
  ...
  const allChapters = [...chapters, ...orphanPyqEntries].sort((left, right) => left.title.localeCompare(right.title));
  ...
  const runbook = {
    ...
    commands: [
      ...
      },
      {
        ...
        output: 'data/raw/BIOLOGY/chapters/*.json',
